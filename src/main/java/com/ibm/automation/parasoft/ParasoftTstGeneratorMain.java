...
/**
 * This class program is the main program which should
 * be called with app.config file path as argument
 * 
 * @author kalpana
 * @version 1.0
 * @since 2017-10-31
...
public class ParasoftTstGeneratorMain {
	...
	public static ConfigurationTO copyRAMLDataToDTO(
			Resource urlEndPointsSubNode, int index) throws Exception {
		...
		List<Response> res1 = urlEndPointsSubNode.methods().get(index)
				.responses();
			...
			if (response.body() != null && !response.body().isEmpty()) {
				if (response.body().get(0).schemaContent() != null) {
					...
					try {
						...
						responseSchemaMap.put(
								response.code().value(),
								JsonGeneratorFromSchema
										.generateInputSampleString(
												response.body().get(0)
														.schemaContent()
														.toString(),
												Util.loadProperties(
						...
						System.out
								.println("Error while reading response body schema content"
										+ e.getMessage());
						e.printStackTrace();
					}
				}
		...
		String inputMethodType = urlEndPointsSubNode.methods().get(index)
				.method();
		configurationTO.setMethod(inputMethodType.toUpperCase());
		configurationTO.setDataSourcePath(Util.loadProperties(
				"DATA_SOURCE_PATH", appConfigPath));
		configurationTO.setEndPointUrl(Util.loadProperties("BASE_URL",
				appConfigPath) + urlEndPointsSubNode.resourcePath());
		configurationTO.setTestPathUrl(Util.loadProperties("BASE_URL",
				appConfigPath) + urlEndPointsSubNode.resourcePath());
		configurationTO.setSecuredBy(urlEndPointsSubNode.methods().get(index)
				.securedBy().get(0).name());
		configurationTO.setValidRange(urlEndPointsSubNode.methods().get(index)
				.responses().get(0).code().toString());
		new ObjectMapper();
		...
		try {
			...
			case "POST":
				...
				if (urlEndPointsSubNode.methods().get(index).body().size() > 0) {
					configurationTO
							.setInputSampleString(JsonGeneratorFromSchema
									.generateInputSampleString(
											urlEndPointsSubNode.methods()
													.get(index).body().get(0)
													.schemaContent(),
											Util.loadProperties(
													"RAML_LOCATION",
													appConfigPath)
			...
			System.out
					.println("Error inside swtich case of method type in method copyRAMLDataToDTO()"
							+ e.getMessage());
		}
		System.out.println(configurationTO.getInputSampleString());
	...
	public static void main(String[] args) throws Exception {
		...
		ArrayList<AppConfigurationPropertiesForDataSheet> appConfigPropertiesForDatasheets = Util
				.loadPropertiesForDataSheets(appConfigPath);

		List<String> filesListWithFullPath = Util
				.ramlFilesWithFullPath(appConfigPath);
		...
		api.resources()
				.forEach(
						(urlEndPointsNode) -> {
							...
							List<Method> methodTypes = urlEndPointsNode
									.methods();
							for (Method methodName : methodTypes) {
								List<AppConfigurationPropertiesForDataSheet> dataSheetsListNodeLevel = appConfigPropertiesForDatasheets
										.stream()
										.filter(p -> p.getEndpointUrl()
												.equals(urlEndPointsNode
														.resourcePath()))
										.collect(Collectors.toList());
								ConfigurationTO configurationTO = null;
								try {
									configurationTO = copyRAMLDataToDTO(
											urlEndPointsNode,
											methodPosition.get());
									configurationTO
											.setDataSource(dataSheetsListNodeLevel
													.stream()
													.map(i -> i
															.getDatasheetName())
													.collect(
															Collectors.toList()));
									configurationTO
											.setProfileMappingID(profileMappingID
													.getAndIncrement() + "");
									configurationTO.setRamlFileName(filesList
											.get(0));
									configurationTO
											.setAppConfigPath(appConfigPath);
									configurationTO
											.setInputTstFile(inputTstFile);
									configurationTO
											.setQueryParameters(urlEndPointsNode
													.methods()
													.get(methodPosition.get())
													.queryParameters());
							...
							urlEndPointsNode
									...
									.forEach(
											(urlEndPointsSubNode) -> {
												List<AppConfigurationPropertiesForDataSheet> dataSheetsListSubNodeLevel = appConfigPropertiesForDatasheets
														.stream()
														.filter(p -> p
																.getEndpointUrl()
																.equals(urlEndPointsSubNode
																		.resourcePath()))
														.collect(
																Collectors
																		.toList());
												System.out
														.println(dataSheetsListSubNodeLevel);
												AtomicInteger methodPosition1 = new AtomicInteger();
												List<Method> methodTypes1 = urlEndPointsSubNode
														.methods();
												for (Method methodName : methodTypes1) {
													ConfigurationTO configurationTOSubNode = null;
													try {
														configurationTOSubNode = copyRAMLDataToDTO(
																urlEndPointsSubNode,
																methodPosition1
																		.get());
														configurationTOSubNode
																.setDataSource(dataSheetsListSubNodeLevel
																		.stream()
																		.map(i -> i
																				.getDatasheetName())
																		.collect(
														...
														configurationTOSubNode
																.setInputTstFile(inputTstFile);
														configurationTOSubNode
																.setQueryParameters(urlEndPointsSubNode
																		.methods()
																		.get(methodPosition1
																				.get())
																		.queryParameters());
		...
		try {
			System.out.println("configurationTOEndPointList"
					+ configurationTOEndPointList);
